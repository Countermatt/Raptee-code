...
RPS::RPS(int rpsTcpPort, std::string bootstrapIP, std::string sgxId)
    : mListener(make_shared<TCPListener>(rpsTcpPort)),
    mRpsTcpPort(rpsTcpPort), mPushReqCnt(0), mPullReqCnt(0), mPingReqCnt(0), mData(), mBrahms(), mCyclonSize(10)
{
...
void RPS::sendingThread()      // initiate RPS Request every RPS_SYNC_TIME
{
  ...
  while(true){
    ...
    ecall_MergeView(al1, bl2, gl3, &mData);
...
void RPS::pushRequest()
{
  ...
  res = ecall_Push_Request(&req, &reqSize, &destIP, &destIPSize, &mData);
...
void RPS::pullRequest()
{
  ...
  //Mise en place de la push request
  res = ecall_Pull_Request(&req, &reqSize, &destIP, &destIPSize, &mData);
  ...
  //Receive RPS Answer
    ...
    res = ecall_Pull_Receive_Reply(requestip, &mData, nodeId, mCyclonSize);
    if (res != 0) {
...
void RPS::samplerRequest(int i)
{
  ...
  if( i >= samplesize){
    ...
    int test = 0;
    //Mise en place de la push request
    res = ecall_Ping(&req, &reqSize, &destIP, &destIPSize, &mData);
    if (res != 0) {
...
void RPS::ReceiveRequest(shared_ptr<TCPConnection> conn)
{
  ...
  if(type == 1){
    ...
    res = ecall_Pull_Reply(&ans, &ansSize, &mData);
    if (req != nullptr) delete[] req;
  ...
  //Push Process
    ...
    auto ip = request[2];
    auto nodeId = request[0];
    ecall_Push_Receive_Request(ip, &mData, nodeId, mCyclonSize);
  }

  //Ping Reply
  else if(type == 2){
    res = ecall_Pong(&ans, &ansSize, &mData);
